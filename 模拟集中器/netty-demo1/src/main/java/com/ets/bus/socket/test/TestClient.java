...
public class TestClient implements Runnable{
    ...
    public static Long deviceId = 1020304050607080l;
    ...
    public void run(){
        try {
            addDeviceId();
            new NettyClientTest().connect(8899, "127.0.0.1",deviceId);
        }catch (Exception e){
            logger.error("**************报错了！**************",e);
        }
...
//        tKeep.start();
    ...
    public byte[] intToByteArray(int i) {
        ...
        private static synchronized void addDeviceId(){
            deviceId++;
            System.out.println("集中器编号为-"+deviceId);
        }
