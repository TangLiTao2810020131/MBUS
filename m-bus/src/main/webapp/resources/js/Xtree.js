...
function layuiXtree(options) {
    ...
    _this._trueContainer = document.getElementById(options.elem); //new
    _this._container = document.createElement("div"); //new 
    _this._container.style.display = 'table'; //new
    _this._trueContainer.appendChild(_this._container); //new
...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="' + d[i].value + '"  name="' + d[i].name + '" title="' + d[i].title + '" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '">';
...
layuiXtree.prototype.Rendering = function () {
    ...
    if (_this._ckall) {
     ...
     //new
	if (_this._trueContainer.offsetWidth < _this._container.offsetWidth) {
	    _this._container.style.width = (_this._trueContainer.offsetWidth + 200) + 'px';
	}
}
