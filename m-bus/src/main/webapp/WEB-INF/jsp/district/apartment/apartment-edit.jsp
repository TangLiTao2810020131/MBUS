...
<div class="layui-form layui-form-pane ml30" style="margin-top: 20px;">
	<div class="layui-form-item">
	    <div class="layui-inline">
	      <label class="layui-form-label">公寓编号</label>
...
<script type="text/javascript">
	...
	layui.use(['jquery', 'layer'], function(){
		...
		,layer = layui.layer;
        //检查公寓编号的唯一性
        function isCheckApartmentCode(obj){
            var num;
            $.ajax({
                type : "POST", //提交方式
                url : "${ctx}/apartment/isCheckApartmentCode.action", //路径
		...
		layui.use('form', function(){  
			...
			form.on('submit(confirmBtn)', function (data) {
                ...
                var code=$("#code").val();
                var name=$("#name").val();
                if(code == '${apartment.code}'){
                    //以下是ajax
                    $.ajax({
                        type: 'POST',
                        data:obj,//参数
                        url: "${ctx}/apartment/update.action",
                        dataType : "json",
                    ...
                    var num=isCheckApartmentCode(obj);
                    if(num != 0 && num != -1) {
                        layer.msg('公寓编码已存在,请重新输入！', {
                            icon: 7,
                            time: 2000 //2秒关闭（如果不配置，默认是3秒）
                        }, function () {
                            $("#code").focus();
