	...
	<div class="tableHtml" style="height: 100%;background: #FFFFFF;">
		<div class="top-btn">
			<div class="layui-btn-group">
				<%--<shiro:hasPermission name="新增公寓信息">--%>
				<button class="layui-btn" id="addBtn">添加</button>
				<%--</shiro:hasPermission>--%>
				<%--<shiro:hasPermission name="编辑公寓信息">--%>
				<button class="layui-btn" id="editBtn">编辑</button>
				<%--</shiro:hasPermission>--%>
				<%--<shiro:hasPermission name="批量删除公寓信息">--%>
				<button class="layui-btn" id="delBtn">删除</button>
				<%--</shiro:hasPermission>--%>
			</div>
			<div class="search-btn-group">
				公寓编号:
				<input type="text" autocomplete="off" class="layui-input layui-input_" id="code" placeholder="请输入公寓编号" style="width:150px;" maxlength="20">
				公寓名称:
				<input type="text" autocomplete="off" class="layui-input layui-input_" id="name" placeholder="请输入公寓名称" style="width:150px;" maxlength="20">
...
<script type="text/javascript">
	...
	layui.use([ 'jquery', 'layer' ], function() {
		...
		var urls = '${ctx}/apartment/listData.action?parentId=${parentid}';
		...
		$("#addBtn").click(function() {
			com.pageOpen("新增", "${ctx}/apartment/add.action?parentid=${parentid}", [ '700px', '380px' ]);
		});
		/*-----编辑-----*/
		$("#editBtn").click(function() {
			com.checkOpen("编辑", "${ctx}/apartment/edit.action", [ '700px', '380px' ]);
		});
        /*----删除之前检测该公寓下的房间是否绑定了水表-------*/
        function isCheckApartmentWaterMeter(ids){
            var num;
            $.ajax({
                type : "POST", //提交方式
                url : "${ctx}/watermeterroominfomgt/isCheckApartmentWaterMeter.action", //路径
        ...
        $("#delBtn").click(function(){
            ...
            layer.confirm("确定删除?删除后不可恢复!", {
                ...
                var num =isCheckApartmentWaterMeter(ids);
                if(num != 0){
                    parent.layer.open({
                        title: '温馨提示'
                        , content: '该公寓下有绑定的水表，不能删除！'
                    })
                    return false ;
                }
                $.ajax({
                    type : "POST", //提交方式
                    url : "${ctx}/apartment/delete.action", //路径
                    data : {
                        "id" : ids
                    }, //数据，这里使用的是Json格式进行传输
                    dataType : "json",
		...
		table.on('tool(tableBox)', function(obj) {
			...
			if (obj.event === 'detail') {
				layer.open({
					...
					fixed : false, //不固定
					maxmin : true,
					content : '${ctx}/apartment/detail.action?id=' + data.id+ '&parentId='+'${parentid}'
				});
