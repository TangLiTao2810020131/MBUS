...
<div class="layui-form layui-form-pane ml30" style="margin-top: 20px;">
	<div class="layui-form-item">
	    ...
	    <div class="layui-inline">
	      ...
	      <div class="layui-input-inline">
	        <input type="text" name="name" id="name"  class="layui-input" lay-verify="required|name" >
	      </div>
	    </div>
  	</div>
...
<script type="text/javascript">
	...
	layui.use(['jquery', 'layer'], function(){
		...
		,layer = layui.layer;
        //检查公寓编号的唯一性
        function isCheckLayerCode(obj){
            var num;
            $.ajax({
                type : "POST", //提交方式
                url : "${ctx}/layer/isCheckLayerCode.action", //路径
			...
			form.on('submit(confirmBtn)', function (data) {
                ...
                if(code == '${layer.code}'){
			     ...
			     $.ajax({
                    ...
                    var num=isCheckLayerCode(obj);
                    if(num != 0 && num != -1) {
                        layer.msg('楼层编号已存在,请重新输入！', {
                            icon: 7,
                            time: 2000 //2秒关闭（如果不配置，默认是3秒）
                        }, function () {
                            $("#code").focus();
