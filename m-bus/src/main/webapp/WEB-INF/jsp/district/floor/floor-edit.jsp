...
<script type="text/javascript">
	...
	layui.use(['jquery', 'layer'], function(){
		...
		,layer = layui.layer;
        //检查公寓编号的唯一性
        function isCheckFloorCode(obj){
            var num;
            $.ajax({
                type : "POST", //提交方式
                url : "${ctx}/floor/isCheckFloorCode.action", //路径
		...
		layui.use('form', function(){  
			...
			form.on('submit(confirmBtn)', function (data) {
                ...
                if(code == '${floor.code}'){
                    ...
                    var num=isCheckFloorCode(obj);
                    if(num != 0 && num != -1) {
                        layer.msg('楼栋编号已存在,请重新输入！', {
                            icon: 7,
                            time: 2000 //2秒关闭（如果不配置，默认是3秒）
                        }, function () {
                            $("#code").focus();
