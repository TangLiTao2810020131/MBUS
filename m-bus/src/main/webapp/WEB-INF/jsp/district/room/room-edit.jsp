			...
			<div class="layui-input-inline" >
				<select name="roomType" id="roomType">
					...
					<c:forEach items="${roomTypeList}" var="roomType">
							<option <c:if test="${roomType.type_num == info.roomTypeId}"> selected="selected" </c:if> value="${roomType.id}" >${roomType.type_name}</option>
					</c:forEach>
				</select>
			</div>
		</div>
			...
			form.on('submit(confirmBtn)', function (data) {
				...
				//以下是ajax
                    $.ajax({
                        type: 'POST',
                        data:obj,//参数
                        url: "${ctx}/room/update.action?roomTypeId="+$('#roomType').val(),
                        dataType : "json",
