...
<div class="tableHtml" style="margin-left: 210px;height: 100%;background: #FFFFFF;">
    <div class="top-btn">
        <div class="layui-btn-group">
            <%--<shiro:hasPermission name="用水房间日报导出">--%>
...
<form class="layui-form" action="" id="formHtml" lay-filter="updForm">
    <div class="layui-form-item">
        ...
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" id="apartmentName" placeholder="请输入公寓名称"  autocomplete="off"
    ...
    <div class="layui-form-item">
        <div class="layui-inline">
            <label class="layui-form-label" title="楼栋">楼栋</label>
            <div class="layui-input-inline">
                <input type="text" id="buildName" placeholder="请输入楼栋名称"  autocomplete="off"
                       class="layui-input"  maxlength="20">
            </div>
        </div>
    </div>
...
<script type="text/html" id="handle">
    <%--<shiro:hasPermission name="查看用水房间日报">--%>
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer'], function () {
        ...
        var arr = [[
            ...
            , {field: 'date', title: '统计日期',templet : "<div>{{layui.util.toDateString(d.createTime, 'yyyy-MM-dd HH:mm:ss')}}</div>"}
            , {field: 'surplusWater', title: '剩余水量'}
            , {field: 'userWater', title: '已用水量'}
            , {field: 'cashBuyWater', title: '当日购水量'}
            , {field: 'supplementWater', title: '当日补水量'}
            , {field: 'returnWater', title: '当日退水量'}
            ,{title: '操作',toolbar:'#handle'}
        ]];
        var limitArr = [10, 20, 30];
        var urls = '${ctx}/roomDaily/listData.action';
        ...
        table.on('tool(tableBox)', function(obj){
            var data = obj.data;
            if(obj.event === 'detail'){
                com.pageOpen("查看房间用水日报","${ctx}/roomDaily/findRoomDaily.action?id="+data.id,['800px', '550px']);
            }
        });
        ...
        $('.search-class').on('click', function(){
            ...
            var tableWhere={
                ...
                ,buildName:$('#buildName').val().replace(/(^\s*)|(\s*$)/g, "")
    ...
    layui.config({
        ...
        $(function(){
            $("#exportBtn").on("click",function(){
                var str="区域,公寓,楼栋,房间号,统计日期,剩余水量,已用水量,当日购水量,当日补水量,当日退水量";
                var name="用水房间日报";
                var url="${ctx}/roomDaily/export.action?str="+str+"&name="+name+"&startTime="+$('#startTime').val()+
                    "&endTime="+$('#endTime').val()+"&roomTypeId="+$('#roomTypeId').val()+
                    "&apartmentName="+$('#apartmentName').val()+"&buildName="+$('#buildName').val()+
                    "&roomNum="+$('#roomNum').val()+ "&level="+$("#level").val()+"&parentId="+$("#parentId").val();
