...
<div class="tableHtml" style="margin-left: 210px;height: 100%;background: #FFFFFF;">
  <div class="top-btn">
		<div class="layui-btn-group">
			<%--<shiro:hasPermission name="购水记录导出">--%>
...
<form class="layui-form" action="" id="formHtml" lay-filter="updForm">
	  ...
	  <div class="layui-form-item">
	      ...
	      <div class="layui-input-inline">
	        <select name="" id="type">
	            ...
	            <option value="0">现金购水</option>
	            <option value="1">一卡通购水</option>
	        </select>
	      </div>
	    </div>
		  <div class="layui-inline">
	...
	<script type="text/html" id="handle">
		<%--<shiro:hasPermission name="查看购水记录">--%>
...
layui.use(['jquery', 'layer','table','tree'], function(){
  ...
  var laydate = layui.laydate;
	...
	var arr = [[ 
        ...
        ,{field: 'create_time', title: '购水日期'}
        ,{field: 'payer_name', title: '购水人姓名'}
        ,{field: 'buy_money', title: '购水金额'}
        ,{field: 'buy_water', title: '购水量'}
        , {field: 'user_name', title: '操作员'}
        ,{title: '操作',toolbar:'#handle'}
    ]];
	var limitArr =[10,20,30];
	var urls = '${ctx}/buyWaterRecord/listData.action';
    ...
    table.on('tool(tableBox)', function(obj){
        var data = obj.data;
        if(obj.event === 'detail'){
            com.pageOpen("查看购水记录","${ctx}/buyWaterRecord/findBuyRecord.action?id="+data.id,['800px', '550px']);
        }
    });
	...
	$(function(){
        $("#exportBtn").on("click",function(){
            var str="区域,公寓,楼层,房间号,操作状态,购水日期,购水人姓名,购水金额（元）,购水量（吨）,操作员";
            var name="购水记录";
            var url="${ctx}/buyWaterRecord/export.action?str="+str+"&name="+name+"&type="+$("#type").val()+
                "&room_num="+$("#room_num").val()+"&startTime="+$("#date2").val()+
                "&endTime="+$("#date1").val()+"&apartment_name="+$("#apartment_name").val()+"&floor_name="+$("#floor_name").val()+
