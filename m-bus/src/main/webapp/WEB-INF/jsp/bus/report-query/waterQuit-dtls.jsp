...
<div class="tableHtml" style="margin-left: 210px;height: 100%;background: #FFFFFF;">
  <div class="top-btn">
		<div class="layui-btn-group">
			<%--<shiro:hasPermission name="退水明细导出">--%>
...
<script type="text/html" id="handle">
	<shiro:hasPermission name="查看退水明细">
...
layui.use(['jquery', 'layer'], function(){
  ...
  var laydate = layui.laydate;
	...
	var arr = [[ 
        ...
        ,{field: 'create_time', title: '退水日期'}
        // ,{field: 'ip_address', title: '集中器'}
        ,{field: 'return_money', title: '退水金额'}
        ,{field: 'return_water', title: '退水量'}
        ,{field: 'user_name', title: '操作员'}
        // ,{field: 'buy_water_total', title: '总购水量（吨）'}
        ,{title: '操作',toolbar:'#handle'}
    ]];
	var limitArr =[10,20,30];
	var urls = '${ctx}/exitWaterRecord/listData.action';
    ...
    table.on('tool(tableBox)', function(obj){
        var data = obj.data;
        if(obj.event === 'detail'){
            com.pageOpen("查看退水记录","${ctx}/exitWaterRecord/findExitRecord.action?id="+data.id,['800px', '550px']);
        }
    });
    ...
    $(function(){
        $("#exportBtn").on("click",function(){
            var str="区域,公寓,楼层,房间号,操作状态,退水日期,退水金额(元),退水量(吨),操作员";
            var name="退水记录";
            var url="${ctx}/exitWaterRecord/export.action?str="+str+"&name="+name+"&room_num="+$("#room_num").val()+
