...
<div class="tableHtml" style="margin-left: 210px;height: 100%;background: #FFFFFF;">
    <div class="top-btn">
        <div class="layui-btn-group">
            <%--<shiro:hasPermission name="历史抄表导出">--%>
            <button class="layui-btn" id="exportBtn"> &nbsp;导 &nbsp;出&nbsp; </button>
           <%-- </shiro:hasPermission>--%>
        </div>
        <div class="search-btn-group">
            <input type="text" name="date"  id="startTime" lay-verify="date" placeholder="请选择开始时间" autocomplete="off" class="layui-input layui-input_">
            <input type="text" name="date"  id="endTime" lay-verify="date" placeholder="请选择结束时间" autocomplete="off" class="layui-input layui-input_">
...
<script type="text/html" id="handle">
    <shiro:hasPermission name="查看历史抄表">
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer'], function(){
        ...
        table.on('tool(tableBox)', function(obj){
            var data = obj.data;
            if(obj.event === 'detail'){
                com.pageOpen("查看历史抄表","${ctx}/waterCollectRecord/findHistoryRecordById.action?id="+data.id,['800px', '550px']);
            }
        });
        ...
        $(function(){
            $("#exportBtn").on("click",function(){
                var str="区域,公寓,楼栋,楼层,房间号,水表编号,操作状态,抄表日期,购水量,补水量,透支水量,总用水量";
                var name="历史抄表记录";
                var url="${ctx}/waterCollectRecord/export.action?str="+str+"&name="+name+"&room_num="+$("#room_num").val()+
