	...
	<div class="tableHtml" style="height: 100%;background: #FFFFFF;">
		<div class="top-btn">
			<div class="layui-btn-group">
				<%--<shiro:hasPermission name="新增楼栋信息">--%>
				<button class="layui-btn" id="addBtn">添加</button>
				<%--</shiro:hasPermission>--%>
				<%--<shiro:hasPermission name="编辑楼栋信息">--%>
				<button class="layui-btn" id="editBtn">编辑</button>
				<%--</shiro:hasPermission>--%>
				<%--<shiro:hasPermission name="批量删除楼栋信息">--%>
				<button class="layui-btn" id="delBtn">删除</button>
				<%--</shiro:hasPermission>--%>
			</div>
			<div class="search-btn-group">
				楼栋编号:
				<input type="text" autocomplete="off" class="layui-input layui-input_" id="code" placeholder="请输入楼栋编号" style="width:150px;" maxlength="20">
				楼栋名称:
				<input type="text" autocomplete="off" class="layui-input layui-input_" id="name" placeholder="请输入楼栋名称" style="width:150px;" maxlength="20">
...
<script type="text/html" id="handle">
	<%--<shiro:hasPermission name="查看楼栋信息">--%>
 	<a lay-event="detail" title="查看" style="cursor:pointer; margin-right:5px;">
		<i class="layui-icon">&#xe63c;</i>
	</a>
	<%--</shiro:hasPermission>
   <shiro:hasPermission name="删除楼栋信息">--%>
...
<script type="text/javascript">
	...
	layui.use([ 'jquery', 'layer' ], function() {
		...
		var urls = '${ctx}/floor/listData.action?parentid=${parentid}';
		...
		$("#addBtn").click(function() {
			com.pageOpen("新增", "${ctx}/floor/add.action?parentid=${parentid}", [ '700px', '380px' ]);
		});
		/*-----编辑-----*/
		$("#editBtn").click(function() {
			com.checkOpen("编辑", "${ctx}/floor/edit.action", [ '700px', '380px' ]);
		});
        /*----删除之前检测该公寓下的房间是否绑定了水表-------*/
        function isCheckFloorWaterMeter(ids){
            var num;
            $.ajax({
                type : "POST", //提交方式
                url : "${ctx}/watermeterroominfomgt/isCheckFloorWaterMeter.action", //路径
        ...
        $("#delBtn").click(function(){
            var checkStatus = table.checkStatus('tableId');//test即为基础参数id对应的
            //数据只能是一条
            ...
            layer.confirm("确定删除?删除后不可恢复!", {
                ...
                var num =isCheckFloorWaterMeter(ids);
                if(num != 0){
                    parent.layer.open({
                        title: '温馨提示'
                        , content: '该楼栋下的房间有绑定的水表，不能删除！'
                    })
                    return false ;
                }
                $.ajax({
                    type : "POST", //提交方式
                    url : "${ctx}/floor/delete.action", //路径
                    data : {
                        "id" : ids
                    }, //数据，这里使用的是Json格式进行传输
                    dataType : "json",
		...
		if (obj.event === 'detail') {
				layer.open({
					...
					fixed : false, //不固定
					maxmin : true,
					content : '${ctx}/floor/detail.action?id=' + data.id+'&parentId='+'${parentid}'
				});
