...
<div class="tableHtml" style="height: 100%;background: #FFFFFF;">
	<div class="top-btn" id="buttons">
		<div class="layui-btn-group" style="position: relative;">
			...
			<button class="layui-btn" data-method="inRole">分配权限</button>
		<div>
    ...
    function tableData() {
        ...
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,cols: [[ //标题栏
                    {type:'checkbox'}
                    ,{field: 'rolename', title: '角色名称', minWidth:120,align : 'center',unresize:true}
                    ,{title: '操作',toolbar:'#handle',align : 'center',unresize:true}
                ]]
                ,url: '${ctx}/rolemgt/listData.action'
                ,id:'myTable'
                ,page: true //是否显示分页
                ,limits: [15,20,50]
                ,limit:15 //每页默认显示的数量
            ...
            active = {
                addUser: function(){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/rolemgt/input.action'
                    });
                },
                inRole:function(){
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/rolemgt/inAuthority.action?id='+data[0].id
                    });
                },
                deleteUsers:function(){
                    ...
                    layer.confirm("删除该角色后,相关用户不可操作，是否确认删除!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
								url : "${ctx}/rolemgt/delete.action",//路径
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'detail'){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/rolemgt/info.action?id='+data.id
                    });
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/rolemgt/input.action?id='+data.id
                    });
                }
            });
