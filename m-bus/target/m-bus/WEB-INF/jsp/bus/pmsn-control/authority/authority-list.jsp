            ...
            active = {
                addUser: function(){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/authorityMgt/input.action'
                    });
                },
                inResource:function(){
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/authorityMgt/inResource.action?id='+data[0].id
                    });
                },
                deleteUsers:function(){
                    ...
                    layer.confirm("删除该权限后,相关用户不可操作，是否确认删除!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/authorityMgt/delete.action",//路径
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'detail'){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/authorityMgt/info.action?id='+data.id
                    });
                } else if(obj.event === 'del'){
                    layer.confirm("确定删除?删除后不可恢复!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.post("${ctx}/authorityMgt/delete.action?id="+data.id);
                        layer.close(index);
                        location.reload();
                    });
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/authorityMgt/input.action?id='+data.id
                    });
                }
            });
