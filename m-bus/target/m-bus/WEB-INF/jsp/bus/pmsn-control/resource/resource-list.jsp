...
<div class="tableHtml" style="height: 100%;background: #FFFFFF;">
    <div class="top-btn" id="buttons">
        <div class="layui-btn-group" style="position: relative;">
            ...
            <button class="layui-btn" data-method="resourceTree">资源树</button>
            <div>
    ...
    function tableData() {
        ...
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,cols: [[ //标题栏
                    {type:'checkbox'}
                ,{field: 'resourcename', title: '资源名称', minWidth:120,align:'center',unresize:true}
                    ,{field: 'resourceurl', title: 'URL',  minWidth:120,align:'center',unresize:true}
                    ,{title: '操作',width:avg,toolbar:'#handle',align:'center',unresize:true}
                ]]
                ,url: '${ctx}/resourceMgt/listData.action'
                ,id:'myTable'
                ,page: true //是否显示分页
                ,limits: [10,20,50]
                ,limit:15 //每页默认显示的数量
            ...
            active = {
                addUser: function(){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/resourceMgt/input.action'
                    });
                },
                deleteUsers:function(){
                    ...
                    layer.confirm("确定删除?删除后不可恢复!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/resourceMgt/delete.action",//路径
                ...
                ,resourceTree: function(){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/resourceMgt/tree.action'
                    });
                }
            };
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'detail'){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/resourceMgt/info.action?id='+data.id
                    });
                } else if(obj.event === 'del'){
                    layer.confirm("确定删除?删除后不可恢复!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.post("${ctx}/resourceMgt/delete.action?id="+data.id);
                        layer.close(index);
                        location.reload();
                    });
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/resourceMgt/input.action?id='+data.id
                    });
                }
            });
